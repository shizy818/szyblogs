...
在本地打印DEBUG日志验证了一下。**有个地方没想明白，为啥ipykernel是ROUTER，jupyter_client是DEALER，不是反了吗？**
```sh
...
**对于一个Jupyter Kernel，不管connection连接数有多少，也不管有多少个Cell运行，代码都是串行执行，为什么呢？**
...
从日志上看，如果两段代码在两个Cell中同时执行，jupyter_client调用了send方法发出了`execute_request`消息；ipykernel在收到第一个消息后调用dispatch_shell函数，然后直到代码运行结束才开始第二个消息的回调函数。但是从代码中，我没有找到zeroMQ或者ipykernel是如何阻塞的。
...
![image04]({{site.baseurl}}/image/20180810/toree_with_notebook.png)
...
![image05]({{site.baseurl}}/image/20180810/interactive_mode.png)
...
![image06]({{site.baseurl}}/image/20180810/kg_basic.png)
...
![image07]({{site.baseurl}}/image/20180810/toree_spark_gateway.png)

## Cell代码块串行执行

和IPython一样，同一个Kernel下多个Cell的代码块执行是串行的。下面的计算PI的示例中，Spark只有一个Job在运行：

![image08]({{site.baseurl}}/image/20180810/notebook_toree.png)

![image09]({{site.baseurl}}/image/20180810/spark_job.png)
