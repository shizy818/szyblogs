...
segment inode用于保存段信息，入下表所示。为了找到segment inode所在位置，还需要一个segment header。对于用户表来说，segment header总算保存在root页中，分别指向叶子节点segment inode和非叶子节点segment inode。函数fseg_create_general创建一个段，初始化segment inode已经segment header.
